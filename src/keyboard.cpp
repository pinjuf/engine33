...
void handle_keyboard() {
    ...
    if (glfwGetKey(window, GLFW_KEY_LEFT_SHIFT)) cam.position += UP * (2.0f * (float)deltaT);
    if (glfwGetKey(window, GLFW_KEY_LEFT_CONTROL)) cam.position -= UP * (2.0f * (float)deltaT);
}
